...
export class RuleEngine {
  ...
  evaluateMessage(message: Message, rules: RuleDoc[]): RuleMatch | null {
    ...
    const messageTargets = new Set([
      AutomodTarget.MESSAGE_CONTENT,
      AutomodTarget.MESSAGE_EMOJI,
      AutomodTarget.STICKER,
      AutomodTarget.LINK,
    ]);

    const applicableRules = rules
      .filter((r) => r.enabled && messageTargets.has(r.target as AutomodTarget))
    ...
    for (const rule of applicableRules) {
      ...
      const result = testPatterns(
        rule.patterns as Array<{ regex: string; flags: string; label: string }>,
        content,
        rule.matchMode as "any" | "all",
      );
  ...
  evaluateReaction(reaction: MessageReaction | PartialMessageReaction, rules: RuleDoc[]): RuleMatch | null {
    const applicableRules = rules
      .filter((r) => r.enabled && r.target === AutomodTarget.REACTION_EMOJI)
      .sort((a, b) => (b.priority ?? 0) - (a.priority ?? 0));
  ...
  evaluateMember(
    ...
    const applicableRules = rules
      .filter((r) => r.enabled && r.target === targetEnum)
      .sort((a, b) => (b.priority ?? 0) - (a.priority ?? 0));

    const content = target === "username"
  ...
  private extractContentForTarget(message: Message, target: AutomodTarget): string | null {
    switch (target) {
      ...
      case AutomodTarget.MESSAGE_EMOJI: {
        ...
        const parts: string[] = [
          ...emojis.unicode,
          ...emojis.custom.map((e) => `${e.name}:${e.id}`),
        ];
