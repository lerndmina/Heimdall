 ...
 *
 * Create or update attachment blocker rules for a TempVC opener.
 *
...
export function createOpenersUpdateRoutes(deps: AttachmentBlockerApiDependencies): Router {
  const router = Router({ mergeParams: true });

  router.put("/:openerChannelId", async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const config = await deps.service.upsertOpenerConfig(guildId, openerChannelId, {
