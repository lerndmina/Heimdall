  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setOriginalConfig(JSON.stringify({
          mode: "disabled",
          provider: "local",
          model: "base.en",
    ...
    return (
      <Card>
        <CardContent>
          ...
          <button
            onClick={fetchConfig}
            className="mt-3 rounded-lg border border-zinc-700/30 px-4 py-2 text-sm font-medium text-zinc-300 transition hover:bg-white/5"
          >
            Retry
          </button>
        </CardContent>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          <StatusBadge variant={mode === "disabled" ? "neutral" : mode === "auto" ? "success" : "info"}>
            {mode === "disabled" ? "Disabled" : mode === "auto" ? "Auto" : "Reactions"}
          </StatusBadge>
        </div>
        <CardDescription className="mt-1">
          Choose how voice messages are transcribed in your server.
        </CardDescription>
        <CardContent className="mt-4">
          <div className="space-y-1.5">
            <p className="text-sm font-medium text-zinc-200">Mode</p>
            <p className="text-xs text-zinc-500">
              <strong>Disabled</strong> — no transcription.{" "}
              <strong>Reactions</strong> — users react with ✍️ to transcribe.{" "}
              <strong>Auto</strong> — all voice messages are transcribed automatically.
            </p>
            <Combobox
      ...
      <Card>
        ...
        <CardDescription className="mt-1">
          Choose between running Whisper locally or using OpenAI&apos;s cloud API.
        </CardDescription>
        <CardContent className="mt-4 space-y-5">
          <div className="space-y-1.5">
            <p className="text-sm font-medium text-zinc-200">Provider</p>
            <p className="text-xs text-zinc-500">
              <strong>Local</strong> — uses whisper.cpp on the server (no API key needed, requires FFmpeg).{" "}
              <strong>OpenAI API</strong> — uses OpenAI&apos;s Whisper API (requires API key, more accurate).
            </p>
            <Combobox
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-zinc-500">
              {provider === "local"
                ...
                : "OpenAI currently offers the whisper-1 model."}
            </p>
            <Combobox
      ...
      {provider === "openai" && (
        <Card>
          <div className="flex items-center justify-between">
            ...
            <StatusBadge variant={hasApiKey ? "success" : "warning"}>
              {hasApiKey ? "Configured" : "Not Set"}
            </StatusBadge>
          </div>
          <CardDescription className="mt-1">
            Your API key is encrypted at rest using AES-256-CBC and never exposed via the API.
          </CardDescription>
              ...
              <div className="space-y-3">
                ...
                {hasApiKey && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 text-xs font-medium text-red-400 transition hover:text-red-300"
                  >
      ...
      <Card>
        ...
        <CardDescription className="mt-1">
          Control which channels voice messages are transcribed in.
        </CardDescription>
        <CardContent className="mt-4 space-y-4">
          <div className="space-y-1.5">
          ...
          {channelFilterMode !== "disabled" && (
            <div className="space-y-3">
              <p className="text-sm font-medium text-zinc-200">
                {channelFilterMode === "whitelist" ? "Allowed Channels" : "Blocked Channels"}
              </p>
              ...
              {channelFilterChannels.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {channelFilterChannels.map((id) => (
                    <span
                      key={id}
                      className="inline-flex items-center gap-1.5 rounded-full border border-zinc-700/30 bg-white/5 px-3 py-1 text-xs text-zinc-300"
                    >
                      #{id}
                      {canManage && (
                        <button
                          onClick={() => removeChannel(id)}
                          className="ml-0.5 rounded-full p-0.5 transition hover:bg-white/10 hover:text-red-400"
                        >
      ...
      <Card>
        ...
        <CardDescription className="mt-1">
          Control which roles can have their voice messages transcribed.
        </CardDescription>
        <CardContent className="mt-4 space-y-4">
          <div className="space-y-1.5">
            <p className="text-sm font-medium text-zinc-200">Filter Mode</p>
          ...
          {roleFilterMode !== "disabled" && (
            <div className="space-y-3">
              <p className="text-sm font-medium text-zinc-200">
                {roleFilterMode === "whitelist" ? "Allowed Roles" : "Blocked Roles"}
              </p>
      ...
      >
        <p className="text-sm text-zinc-400">
          Are you sure you want to remove the OpenAI API key? Voice transcription will stop working if the provider is set to OpenAI.
        </p>
      </Modal>
