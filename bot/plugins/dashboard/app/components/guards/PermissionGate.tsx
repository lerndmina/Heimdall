...
export default function PermissionGate({ category, children }: PermissionGateProps) {
  const { permissions, isOwner, isBotOwner, isAdministrator, denyAccess, loaded } = usePermissions();
  ...
  // Discord administrators bypass dashboard access requirement unless explicitly denied
  if (isAdministrator && !denyAccess) return <>{children}</>;

  // Require explicit dashboard access
  if (permissions["dashboard.can_access"] !== true) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="mt-1 text-sm text-zinc-500">You don't have permission to access the dashboard.</p>
      </div>
    );
  }

  // Check if the user has any non-command permission in the category
  const commandPrefix = `${category}.commands.`;
  const hasAccess = Object.entries(permissions).some(([key, val]) => key.startsWith(category + ".") && !key.startsWith(commandPrefix) && val === true);
