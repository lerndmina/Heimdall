      ...
      {searchKeys.length > 0 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-zinc-700/30 bg-white/5 py-2 pl-10 pr-4 text-sm text-zinc-100 placeholder-zinc-500 outline-none backdrop-blur-sm transition focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
          />
        </div>
      )}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map((row, i) => (
                <tr key={i} className="border-b border-zinc-700/20 transition hover:bg-white/5">
                  {columns.map((col) => (
