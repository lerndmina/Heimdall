/**
 * Dashboard Plugin — Next.js web dashboard served from its own port.
 *
...
export async function onLoad(context: PluginContext): Promise<DashboardPluginAPI> {
  ...
  // ── Write .env.local so Next.js can see the bot's env vars ──────────
  ...
  const lines: string[] = ["# Auto-generated by dashboard plugin — do not edit manually", `# Generated at ${new Date().toISOString()}`];

  for (const key of allKeys) {
    let value = process.env[key];

    // Map NEXTAUTH_SECRET → AUTH_SECRET (NextAuth v5 expects AUTH_SECRET)
