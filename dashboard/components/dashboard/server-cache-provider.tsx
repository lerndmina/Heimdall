...
export function ServerCacheProvider({ children, userId }: ServerCacheProviderProps) {
  ...
  const loadServers = async (userIdToUse?: string) => {
  ...
  const loadServersFromAPI = async (targetUserId: string) => {
    try {
  ...
  const initializeCache = async (newUserId: string) => {
    setCurrentUserId(newUserId);
    await loadServers(newUserId);
  };

  const getServerById = (id: string): Guild | undefined => {
    return servers.find((server) => server.id === id);
  };

  const checkServerPermissions = async (id: string): Promise<boolean> => {
    try {
