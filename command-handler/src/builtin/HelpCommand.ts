...
export class HelpCommand {
  ...
  getHelpCommand(): LoadedCommand {
    const data = new SlashCommandBuilder()
      ...
      .addBooleanOption(option =>
        option
          .setName("show-dev")
          .setDescription("Include development-only commands (if you have access)")
          .setRequired(false)
      );
  ...
  private async showCommandDetails(interaction: ChatInputCommandInteraction, commandName: string, showDev: boolean): Promise<void> {
    ...
    if (applicationCommand) {
      embed.addFields([
        { name: "Usage", value: `</${commandName}:${applicationCommand.id}>`, inline: false }
      ]);
    }
    ...
    if (permissions) {
      embed.addFields([
        { name: "Permissions Required", value: permissions, inline: false }
      ]);
    }
    ...
    if (subcommands.length > 0) {
      ...
      embed.addFields([
        { name: "Subcommands", value: subcommandText.slice(0, 1024), inline: false }
      ]);
    }
  ...
  private async showPaginatedEmbeds(interaction: ChatInputCommandInteraction, pages: EmbedBuilder[], title: string): Promise<void> {
    ...
    const prevButton = new ButtonKit()
      ...
      .setCustomId(`help-prev-${interactionId}`)
      .setDisabled(true);

    const homeButton = new ButtonKit()
      .setEmoji("🏠")
      .setStyle(ButtonStyle.Secondary)
      .setCustomId(`help-home-${interactionId}`)
      .setDisabled(true);
    ...
    // Button handlers
    prevButton.onClick((btnInteraction) => {
      setCurrentPage(prev => Math.max(0, prev - 1));
      btnInteraction.deferUpdate().catch(console.error);
    }, { message });

    homeButton.onClick((btnInteraction) => {
      setCurrentPage(0);
      btnInteraction.deferUpdate().catch(console.error);
    }, { message });

    nextButton.onClick((btnInteraction) => {
  ...
  private createCategoryPages(category: CategoryInfo, commands: CommandInfo[]): EmbedBuilder[] {
    ...
    for (let i = 0; i < commands.length; i += commandsPerPage) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`📂 ${category.displayName} Commands`)
        .setColor(Colors.Blue)
        .setTimestamp();
  ...
  private isGuildOnly(command: LoadedCommand): boolean {
    if (command.data instanceof SlashCommandBuilder) {
      ...
      return (dmPermission === false) || 
        (json.contexts && !json.contexts.includes(1) && !json.contexts.includes(2)) ||
        false;
    }
    return false;
  }
